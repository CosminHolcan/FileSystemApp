...
namespace BLL
{
    public class FileVersionsBLL
    {
        ...
        public async Task<FileVersionDTO> AddVersion(Guid userId, AddFileVersionDTO dto, IFormFile file)
        {
            ...
            try
            {
                ...
                toReturn = new FileVersionDTO
                {
                    ...
                    CreationTime = Helper.FormatDateTime(createdVersion.CreationTime),
                    TokenSAS = await SASTokensGenerator.GenerateSasTokenAsync(appFile.StorageAccount.BlobServicePath, "container", Helper.GetAzureFileName(appFile.Id, appFile.Name), createdVersion.AzureId)
                };
                ...
                if (appFile.ReplicaId != null)
                {
                    ...
                    if (firstFileFailed)
                    {
                        toReturn = new FileVersionDTO
                        {
                            ...
                            OriginalFileId = createdVersion.OriginalFileId,
                            CreationTime = Helper.FormatDateTime(createdVersion.CreationTime),
                            TokenSAS = await SASTokensGenerator.GenerateSasTokenAsync(replicaFile.StorageAccount.BlobServicePath, "container", Helper.GetAzureFileName(replicaFile.Id, replicaFile.Name), createdVersion.AzureId)
                        };
        ...
        public async Task<List<FileVersionDTO>> GetFileVersionsByOriginalFileId(AppFile appFile)
        {
            ...
            foreach (var f in fileVersions)
            {
                ...
                result.Add(new FileVersionDTO()
                {
                    ...
                    CreationTime = Helper.FormatDateTime(f.CreationTime),
                    AzureId = f.AzureId,
                    TokenSAS = token
                });
            }
        ...
        private async Task<string> UploadFileToBlob(AppFile appFile, IFormFile file)
        {
            ...
            {
                ContentType = Helper.GetContentType(appFile.Name),
        ...
        private async Task DeleteFileVersionFromAzure(AppFile appFile, FileVersion fileVersion)
        {
            ...
            BlobClient blobClient = containerClient.GetBlobClient(Helper.GetAzureFileName(appFile.Id, appFile.Name)).WithVersion(fileVersion.AzureId);
